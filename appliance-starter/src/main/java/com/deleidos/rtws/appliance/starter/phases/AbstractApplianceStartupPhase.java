/**
 * LEIDOS CONFIDENTIAL
 * __________________
 *
 * (C)[2007]-[2014] Leidos
 * Unpublished - All Rights Reserved.
 *
 * NOTICE:  All information contained herein is, and remains
 * the exclusive property of Leidos and its suppliers, if any.
 * The intellectual and technical concepts contained
 * herein are proprietary to Leidos and its suppliers
 * and may be covered by U.S. and Foreign Patents,
 * patents in process, and are protected by trade secret or copyright law.
 * Dissemination of this information or reproduction of this material
 * is strictly forbidden unless prior written permission is obtained
 * from Leidos.
 */
package com.deleidos.rtws.appliance.starter.phases;

import org.apache.log4j.Logger;

import com.deleidos.rtws.appliance.starter.exception.ConnectionFailedException;
import com.deleidos.rtws.appliance.starter.model.ApplianceStartupPhase;
import com.deleidos.rtws.appliance.starter.model.PhaseResult;
import com.deleidos.rtws.appliance.starter.model.vmware.HostExecutionEnvironment;
import com.deleidos.rtws.appliance.starter.util.ServiceInstanceBuilder;
import com.vmware.vim25.mo.ServiceInstance;

public abstract class AbstractApplianceStartupPhase implements ApplianceStartupPhase {

	protected HostExecutionEnvironment environment;
	private Logger log = Logger.getLogger(AbstractApplianceStartupPhase.class);
	protected PhaseResult result = new PhaseResult();
	protected ServiceInstance si;

	@Override
	public void initialize(HostExecutionEnvironment environment) {
		if (this.environment == null)
			this.environment = environment;

		if (!Boolean.getBoolean("RTWS_TEST_MODE") && si == null)
			try {
				si = ServiceInstanceBuilder.build(environment);
			} catch (ConnectionFailedException e) {
				log.error(e.getMessage(), e);
			}
	}

	@Override
	public void dispose() {
		si.getServerConnection().logout();
	}
}
