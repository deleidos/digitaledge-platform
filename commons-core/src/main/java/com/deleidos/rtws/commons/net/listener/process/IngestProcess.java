...
public class IngestProcess {
	...
	private static final String INGEST_NOT_INSTALLED = "Ingest server is not installed on this instance.";
	...
	private IngestStatus getStatusHelper() throws ServerException {
		...
		if (! SoftwareUtil.isIngestInstalled()) {
			throw new ServerException(INGEST_NOT_INSTALLED);
		}
		
		IngestStatus status = IngestStatus.Unknown;
		
		String command = "sudo -u rtws /usr/bin/jps | grep IngestManager";
	...
	public PipelineStatus getPipelineStatus() throws ServerException {
		...
		if (SoftwareUtil.isInactive(SoftwareUtil.INGEST_LOGS_DIR, SoftwareUtil.INGEST_INACTIVE_LOGS_FILENAME)) {
