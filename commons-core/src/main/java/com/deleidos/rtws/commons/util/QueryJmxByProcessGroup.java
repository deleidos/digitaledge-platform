...
public class QueryJmxByProcessGroup {
	...
	public static void main(String[] args) {
		...
		if(attributeName==null)
			System.out.println(qg.getAllAttributesForObject(hostAndPort,objectName));
		else
			System.out.println(qg.getCount(hostAndPort, objectName, attributeName));
	}

	private String getAllAttributesForObject(String hostAndPort, String objectName) {
		StringBuilder attributes = new StringBuilder();
		JmxConnection connection = new JmxConnection();
		connection.setJmxUrl("service:jmx:rmi:///jndi/rmi://" + hostAndPort + "/jmxrmi");
		try {
			MBeanServerConnection jmx = connection.connectToBeanServer();
			MBeanInfo info = jmx.getMBeanInfo(new ObjectName(objectName));
			MBeanAttributeInfo[] list = info.getAttributes();
			
			for (MBeanAttributeInfo attribute : list) {
				System.out.println(attribute.getName() + " -> " + jmx.getAttribute(new ObjectName(objectName), attribute.getName()));
			}
