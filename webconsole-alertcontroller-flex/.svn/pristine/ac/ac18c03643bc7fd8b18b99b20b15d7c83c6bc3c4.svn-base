...
package com.deleidos.rtws.alertcontroller.commands
{
	...
	import com.deleidos.rtws.alertcontroller.models.ParameterModel;
...
//		public var paramModel:ParameterModel;
		...
		override public function execute():void 
		{
			...
			currOpTrackingToken = UIDUtil.createUID();
			var loadSubscribersConfig:DefaultOperationConfig =
				new DefaultOperationConfig(
					new LoadGlobalModelEvent(LoadGlobalModelEvent.LOAD_SUBSCRIBERS, null, currOpTrackingToken),
					new TrackableOperationMonitor(eventDispatcher, LoadGlobalModelEvent.LOAD_SUBSCRIBERS_STATUS, currOpTrackingToken),
					15000, 2, true);
			startupSyncOpConfigs.addItem(loadSubscribersConfig);
			
			
			var startupAsyncOpConfigs:ArrayCollection = new ArrayCollection();
			var asyncLoadConfig:DefaultOperationConfig;

			// Setup the current user as an alert subscriber.
			// Either set the values from the subscribers service or make this user a subscriber
			currOpTrackingToken = UIDUtil.createUID();
			asyncLoadConfig =
				new DefaultOperationConfig(
					new LoadGlobalModelEvent(LoadGlobalModelEvent.SETUP_ALERTING_USER, null, currOpTrackingToken),
					new TrackableOperationMonitor(eventDispatcher, LoadGlobalModelEvent.SETUP_ALERTING_USER_STATUS, currOpTrackingToken),
					15000, 2, true);
			setAsyncOpParams(asyncLoadConfig);
			startupAsyncOpConfigs.addItem(asyncLoadConfig);
			
			// Load all system data models
			currOpTrackingToken = UIDUtil.createUID();
			asyncLoadConfig =
				new DefaultOperationConfig(
					new LoadGlobalModelEvent(LoadGlobalModelEvent.LOAD_DATA_MODELS, null, currOpTrackingToken),
					new TrackableOperationMonitor(eventDispatcher, LoadGlobalModelEvent.LOAD_DATA_MODELS_STATUS, currOpTrackingToken),
					15000, 2, true);
			setAsyncOpParams(asyncLoadConfig);
			startupAsyncOpConfigs.addItem(asyncLoadConfig);
			...
			_startupController = new BatchOperationController(eventDispatcher, startupSyncOpConfigs, startupAsyncOpConfigs, AlertController.STARTUP_TRACKING_TOKEN);
		...
		{
			if (event == null || StringUtil.stringHasValue(event.subType) == false || AlertController.STARTUP_TRACKING_TOKEN !== event.trackingToken) 
			{
				return;
			}
			
			if (OperationStatusEvent.SUB_TYPE_SUCCESS == event.subType || OperationStatusEvent.SUB_TYPE_FAILURE == event.subType) 
			{
