...
public class ElasticStorageMonitor extends ProcessMonitor {
	
	public ElasticStorageMonitor(String name) {
		super(name);
	...
	protected void monitor() {
		...
		try {
			URL url = new URL("localhost:9200/_cat/health");
			
			BufferedReader reader = new BufferedReader(new InputStreamReader(url.openStream(), "UTF-8"));
			//example header/line
			//epoch      timestamp cluster                                         status node.total node.data shards pri relo init unassign
			//1404740191 13:36:31  EStestProcChanges9.aws-dev.aws-dev.deleidos.com green           4         2     10   5    0    0        0
		    for (String line; (line = reader.readLine()) != null;) {
		        System.out.println(line);
		        String domain = UserDataProperties.getInstance().getString(UserDataProperties.RTWS_DOMAIN);
		        line = line.substring(line.indexOf(domain)+domain.length()+1);
		        String status = line.substring(0,line.indexOf(' '));
		        if(!status.equalsIgnoreCase("green")){
		        	addError("Cluster status is "+status);
		        }else{
		        	return;
		        }
			...
			//this one will not happen = fixed url
		} catch (Exception e){
