...
public class JettyProcessCommandClient extends SingleTargetCommandClient {
	
	public static JettyProcessCommandClient newInstance(String hostname, int port, String [] args) {
		return new JettyProcessCommandClient(hostname, port, args);
	}

	private JettyProcessCommandClient(String hostname, int port, String [] args) {
	...
	protected boolean isCommandValid() {
		...
		if (command.equals(Command.START_JETTY) ||
			command.equals(Command.STOP_JETTY) ||
			command.equals(Command.RESTART_JETTY) ||
			command.equals(Command.RESTART_JETTY_WITH_DEBUG)) {
			return true;
		}
	...
	private static void validArguments(String [] args) {
		...
		if (args.length != 1 && args.length != 2) {
			System.out.println("Usage: JettyProcessCommandClient <command> [target1,target2,...]");
			System.exit(1);
		}
	...
	public static void main(String [] args) {
		...
		try {
			...
			JettyProcessCommandClient client = JettyProcessCommandClient.newInstance(hostname, port, args);
			client.sendCommand();
		} catch (Exception ex) {
