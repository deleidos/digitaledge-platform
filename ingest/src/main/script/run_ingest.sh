...
# RTWS_INGEST_CONFIG - The name of the configuration file with which to start the ingest process.
#
# Optional parameters.
# RTWS_INGEST_OPTIONS - May be used to specify JVM command line parameters.

source /etc/rtwsrc

if [ ! -d /usr/local/rtws/ingest/logs ]; then
	mkdir -p /usr/local/rtws/ingest/logs
fi
...
ct=$(grep -wc "rtws.cloud.provider=APPLIANCE" /usr/local/rtws/properties/rtws-properties)
if [ $ct >= 1 ]; then
...
if [ ! -e /tmp/ingest/lib ]; then
    mkdir -p /tmp/ingest/lib
fi

if [ ! -e /tmp/ingest/scripts ]; then
    mkdir -p /tmp/ingest/scripts
fi

chmod 777 -R /tmp/ingest/
...
java -Xmx${jvmsize}m -Dlog4j.configuration=file:///usr/local/rtws/ingest/conf/log4j.properties $RTWS_INGEST_OPTIONS -cp $CLASSPATH_MOD -Djava.library.path=../lib/native com.deleidos.rtws.core.framework.IngestManager ../conf/$RTWS_INGEST_CONFIG