 ...
 */
package com.deleidos.rtws.commons.net.listener.executor;
...
import com.deleidos.rtws.commons.monitor.util.MonitorUtil;
import com.deleidos.rtws.commons.net.listener.common.CommandListenerMonitorAccess;
...
import com.deleidos.rtws.commons.net.listener.status.ExecuteResult;
import com.deleidos.rtws.commons.net.listener.util.SoftwareUtil;
import com.deleidos.rtws.commons.util.SelfTerminatingTask;
import com.deleidos.rtws.commons.util.SelfTerminationQueue;
...
public class ShutdownCommandExecutor extends CommandExecutor {
	...
	private Timer timer = new Timer("self-terminating-timer");
	...
	private ShutdownCommandExecutor(String command, String request, OutputStream os) {
		...
		timer.scheduleAtFixedRate(new SelfTerminatingTask(), TimeUnit.MINUTES.toMillis(10l), TimeUnit.MINUTES.toMillis(10l));
	}
	...
	private ExecuteResult handleShutdownSuccess() {
		...
		if (Boolean.parseBoolean(RtwsConfig.getInstance().getString("rtws.auto.terminate.pipeline", "false"))
				&& RtwsConfig.getInstance().getString("rtws.cloud.provider", "UNKNOWN").equals("AWS")
				&& MonitorUtil.isIngestActivityMonitorRegistered()) {
			
			// schedule the termination of this instance 
			SelfTerminationQueue.instance.queue();
		}
