...
public abstract class AbstractApiRestClient {
	...
	protected static String GW_HOST;
	...
	protected AbstractApiRestClient() {
		...
		if (GW_HOST == null) {
			try {
				GW_HOST = RtwsConfig.getInstance().getString("tenant.gateway.host");
			} catch (ConversionException e) {
				logger.fatal("Load property [tenant.gateway.host] failed. Message: " + e.getMessage());
			}
		}
	}
