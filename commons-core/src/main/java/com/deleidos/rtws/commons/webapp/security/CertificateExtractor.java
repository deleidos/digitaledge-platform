 ...
 */
package com.deleidos.rtws.commons.webapp.security;
	...
	public static String getTenantId(X509Certificate certificate) throws CertificateEncodingException {
		...
		for (Object oid : principal.getOIDs()) {
			...
			if (oid instanceof ASN1ObjectIdentifier) {
				ASN1ObjectIdentifier aoid = (ASN1ObjectIdentifier) oid;
				if (aoid.getId().equals(TENANT_ID_OBJECTID)) {
					Vector values = principal.getValues();
					if (values.size() > 0 && values.get(values.size() - 1) instanceof String) {
						return (String) values.get(values.size() - 1);
					}
				}
			}
		}
		log.error("Unable to validate username in certificate.");		
		return null;	
	}
