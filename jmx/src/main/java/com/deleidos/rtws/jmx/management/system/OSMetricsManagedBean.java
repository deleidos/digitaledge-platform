...
import com.deleidos.rtws.commons.management.OSMetricsMXBean;
import com.deleidos.rtws.commons.model.jmx.FileSystemStatBean;

public class OSMetricsManagedBean implements OSMetricsMXBean {

	/** The object that stores the system metrics */
	private Sigar sigar;
	
	public OSMetricsManagedBean() {
	...
	public double [] getLoadAverage() throws SigarException {
		// load average in 1 min, 5 min, and 15 min interval
		return this.sigar.getLoadAverage();
	}
	
	public CpuInfo getCpuInfo() throws SigarException {
		return (this.sigar.getCpuInfoList())[0];
	}
	...
	public List<FileSystemStatBean> getFileSystemStat() throws SigarException {
		List<FileSystemStatBean> fsStatList = new LinkedList<FileSystemStatBean>();
		
		FileSystem [] fsList = this.sigar.getFileSystemList();
		for (FileSystem fs : fsList) {
			FileSystemUsage fsUsage = sigar.getFileSystemUsage(fs.getDirName());
			
			FileSystemStatBean fsStat = new FileSystemStatBean(fs.getDevName(), fs.getDirName(), 
					fs.getSysTypeName(), fs.getTypeName(), fsUsage.getDiskReads(), fsUsage.getDiskReadBytes(),
					fsUsage.getDiskWrites(), fsUsage.getDiskWriteBytes());
			
			fsStatList.add(fsStat);
		}
