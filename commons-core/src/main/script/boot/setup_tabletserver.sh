...
# Set this instance up to be a slave
export hst="$(curl -s -f http://169.254.169.254/latest/meta-data/local-hostname)"
echo -e "$hst" > /usr/lib/accumulo-1.4.1/conf/slaves
echo -e "$hst" > /usr/lib/accumulo-1.4.1/conf/tracers
echo "" > /usr/lib/accumulo-1.4.1/conf/masters
echo "" > /usr/lib/accumulo-1.4.1/conf/gc
echo "" >  /usr/lib/accumulo-1.4.1/conf/monitor


# Wait until the Accumulo cluster is initialized
while (true)
do
	...
	if [ $? -eq 0 ]; then
		echo "Accumulo has been initialized, starting tablet server daemon...."
		sleep 1m
		break 2;
	fi
	sleep 1m
