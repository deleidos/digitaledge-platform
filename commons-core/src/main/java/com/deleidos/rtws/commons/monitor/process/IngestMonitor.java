...
public class IngestMonitor extends ProcessMonitor {
	...
	public IngestMonitor(String name) {
		super(name);
		setStartupPeriod(1000 * 60 * 10);  // Give the ingest process 10 minutes to start
	...
	@Override
	protected void monitorPipeline() {
		try {
			// Get the pipeline status of the transport
			PipelineStatus pipelineStatus = ingestProcess.getPipelineStatus();
