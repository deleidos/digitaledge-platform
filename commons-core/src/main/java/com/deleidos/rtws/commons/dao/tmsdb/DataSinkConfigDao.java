...
public class DataSinkConfigDao {
	...
	private String tableName = "application.datasink_config";
	...
	public Collection<DataSinkConfig> getDataSinkConfigs() {
		Collection<DataSinkConfig> configs = session.executeMultiRowQuery("select * from datasink_config",
				new DefaultStatementHandler(), new DataSinkConfigBuilder());
	...
	protected static final class DataSinkConfigBuilder implements RecordBuilder<DataSinkConfig> {
		public DataSinkConfig buildRecord(ResultSet rs) {
			try {
				...
				// non null field
				config.setCanAutoScale(rs.getString("can_autoscale").charAt(0));
				config.setFqn(rs.getString("fqn"));
				config.setProcessGroupDependencies(rs.getString("process_group_dependencies"));
				config.setNumThreads(rs.getInt("num_threads"));
				config.setUsesBlockStorage(rs.getString("uses_block_storage").charAt(0));
	...
	public void insert(String fqn, boolean scalable, boolean usesEbs, double scalingFactor, String process_group_dependencies) {

		char canScale = scalable == true ? 'Y' : 'N';
		char requiresEbs = usesEbs == true ? 'Y' : 'N';

		session.executeStatement(
				String.format(
						"insert into %s (fqn,can_autoscale,uses_block_storage,scale_up_factor,process_group_dependencies) VALUES (?,?,?,?,?)",
						tableName), new DefaultStatementHandler(fqn, canScale, requiresEbs, scalingFactor,
						process_group_dependencies));

	}

	public void delete(String fqn) {
		session.executeStatement(String.format("delete from %s where fqn = ?", tableName), new DefaultStatementHandler(fqn));
