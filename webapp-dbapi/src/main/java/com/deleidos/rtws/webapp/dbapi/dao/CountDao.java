...
public class CountDao {
	...
	private static Logger log = Logger.getLogger(CountDao.class);
	...
	public boolean createSequence(String name){
		try{
			String sql = "CREATE SEQUENCE "+name+" CACHE 100";
			session.executeStatement(sql, null);
			return true;
		}catch(Exception e){
			return false;
		}
	...
	public boolean createIndex(String type, String table, String jsondef) throws Exception {
		try{
			JSONObject def = JSONObject.fromObject(jsondef);
			StringBuffer sql = new StringBuffer("CREATE ");
			if(def.getBoolean("isUnique"))
				sql.append("UNIQUE ");
			sql.append("INDEX "+def.getString("name"));
			
			sql.append(" ON "+type+"."+table+"(");
			String sep = "";
			JSONArray arr = def.getJSONArray("columns");
			for(int i=0;i<arr.size();i++){
				sql.append(sep);
				sep = ",";
				sql.append(arr.getString(i));
			}
			sql.append(");");
			session.executeStatement(sql.toString(), null);
			return true;
		}catch(Exception e){
			log.warn("Failed to create the index", e);
			throw e;
		}
	}

	public boolean dropIndex(String type, String indName){
		try{
			String sql = "DROP INDEX " + type+"."+indName;
			session.executeStatement(sql, null);
			return true;
		}catch(Exception e){
			return false;
