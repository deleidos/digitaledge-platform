...
package com.deleidos.rtws.commons.view.component
{
	...
	import com.deleidos.rtws.commons.view.component.progressSpinnerClasses.Tick;
    ...
    private var creation:Boolean = true;
		...
		/**
		 * Set the height and width based on the size of the ProgressSpinner. This should be more robust, but oh well.
		 */
		override protected function measure():void {
			measuredMinWidth = _size;
			measuredMinHeight = _size;
			measuredWidth = _size;
			measuredHeight = _size;
		}
		 ...
		 */
		 override protected function updateDisplayList(unscaledWidth:Number, unscaledHeight:Number):void {
		 	if (tickColorChanged || numTicksChanged || sizeChanged || tickWidthChanged || creation) {
				...
				var maxDiameter:int = Math.floor(Math.min(unscaledWidth, unscaledHeight));
				var radius:Number = maxDiameter / 2;
				var angle:Number = 2 * Math.PI / _numTicks; // The angle between each tick
				var tickWidth:Number = (_tickWidth != -1) ? _tickWidth : maxDiameter / 10;
				...
				for (var j:int = 0; j < _numTicks; j++) {
					...
					var xOffset:Number = Math.floor((unscaledWidth - maxDiameter) / 2);
					var yOffset:Number = Math.floor((unscaledHeight - maxDiameter) / 2);
					
					var t:Tick = new Tick(xStart + xOffset, yStart + yOffset, xEnd + xOffset, yEnd + yOffset, tickWidth, tickColor);
						t.alpha = 0.1;
		...
		private static function classConstruct():Boolean {
			if (!StyleManager.getStyleManager(null).getStyleDeclaration("ProgressSpinner")) {
				// If there is no CSS definition for StyledRectangle, 
				// then create one and set the default value.
				var newStyleDeclaration:CSSStyleDeclaration = new CSSStyleDeclaration();
				newStyleDeclaration.setStyle(STYLE_TICK_COLOR, 0x000000);
				StyleManager.getStyleManager(null).setStyleDeclaration("ProgressSpinner", newStyleDeclaration, true);
			}
			return true;
		}
		...
		override public function styleChanged(styleProp:String):void {
			...
			if(styleProp == null || styleProp == STYLE_TICK_COLOR) {
		...
		public function stop():void {
			if (fadeTimer != null && fadeTimer.running) {
				_isPlaying = false;
				fadeTimer.stop();
				fadeTimer.removeEventListener(TimerEvent.TIMER, onFadeTimer);
				fadeTimer = null;
			}
		}
