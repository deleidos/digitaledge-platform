 ...
 */
package com.deleidos.rtws.commons.net.listener.common;
...
import com.deleidos.rtws.commons.management.InitializableManagementContext;
import com.deleidos.rtws.commons.monitor.core.PipelineStatusMonitor;
import com.deleidos.rtws.commons.monitor.core.ProcessGroupMonitor;
import com.deleidos.rtws.commons.monitor.core.SystemStatusMonitor;
import com.deleidos.rtws.commons.monitor.process.NonfatalErrorMonitor;
import com.deleidos.rtws.commons.net.listener.util.UserDataUtil;
...
public class CommandListenerMonitorAccess {
	...
	private static PipelineStatusMonitor pipelineStatusMonitor = null;
	
	private static NonfatalErrorMonitor nonfatalStatusMonitor = null;
	...
	private void initializeJmxContext() {
		...
		String jmxPort = System.getProperty("com.deleidos.rtws.management.commandListener.jmxPort", "61515");
		String rmiPort = System.getProperty("com.deleidos.rtws.management.commandListener.rmiPort", "1098");
		String connectorName = System.getProperty("com.deleidos.rtws.management.connectorName", "jmxrmi");
		
		// Make sure it is managed by JMX
		jmx = new InitializableManagementContext(jmxPort, rmiPort, connectorName, "rtws.deleidos.com");
		jmx.initialize();
	...
	public void startSystemStatusMonitor() {
		if(systemStatusMonitor == null) {
			...
			String host = UserDataUtil.getHostFromFqdn();
			if(host.equals("master") || host.equals("tmsp")) {
	...
	public void startPipelineStatusMonitor() {
		if(pipelineStatusMonitor == null) {
			// Only allow the pipeline status monitor to run if this is the master node
			String host = UserDataUtil.getHostFromFqdn();
			if(host.equals("master") || host.equals("tmsp")) {
				// Get the domain of the system passed in with user data
				String name = UserDataProperties.getInstance().getString(UserDataProperties.RTWS_DOMAIN);
				...
				// Setup the monitor
				pipelineStatusMonitor = new PipelineStatusMonitor("pipeline-" + name);
				
				// Register the monitor with JMX
				jmx.register(pipelineStatusMonitor);
				
				// Start the monitor
				pipelineStatusMonitor.start();
				
				logger.info("Pipeline status monitor registered and started.");
			}
		}
		else {
			logger.info("Unable to start pipeline status monitor - monitor is already running.");
		}
	}
	
	public void startNonfatalMonitor() {
		
		if(nonfatalStatusMonitor == null) {
			...
			nonfatalStatusMonitor = new NonfatalErrorMonitor("nonfatal-"+name);
			jmx.register(nonfatalStatusMonitor);  // Register the monitor with JMX
			nonfatalStatusMonitor.start();
			
			logger.info("Nonfatal error monitor registered and started.");
		}
		else {
			logger.info("Unable to start nonfatal error monitor - monitor is already running.");
		}
	}
	...
	public void startProcessGroupMonitor() {
		if(processGroupMonitor == null) {
			...
			// Setup the monitor instance
			processGroupMonitor = new ProcessGroupMonitor("processgroup-"+name);
			...
			logger.info("Unable to start process group monitor - monitor is already running.");
		}
	}
	...
	public void shutdownProcessGroupMonitor() {
		if(processGroupMonitor != null) {
			...
			logger.info("Unable to stop process group monitor - monitor is not running.");
		}
	}
