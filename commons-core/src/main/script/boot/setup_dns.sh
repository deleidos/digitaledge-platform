...
if [ -n "$RTWS_CUSTOMER_DNS_SERVER_IP" ]; then 
  ...
  if [ -f /etc/dhcp/dhclient.conf ]; then
  	...
  	echo "prepend domain-name-servers $RTWS_CUSTOMER_DNS_SERVER_IP;" >> /etc/dhcp/dhclient.conf
  fi
fi
...
if [ -f /etc/dhcp/dhclient.conf ]; then
	echo "prepend domain-name-servers $RTWS_DNS_ADDRESS;" >> /etc/dhcp/dhclient.conf
fi
...
if [ -f /etc/init.d/bind9 ]; then
    ...
    if [ $? -ne 0 ]; then
    	echo "ERROR: bind is not running, attempting to restart it."
