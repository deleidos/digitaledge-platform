...
public class TransportProcess {
	...
	private static final String TRANSPORT_NOT_INSTALLED = "Transport server is not installed on this instance.";
	...
	public TransportStatus getStatus() throws ServerException {
		...
		if (! SoftwareUtil.isTransportInstalled()) {
			throw new ServerException(TRANSPORT_NOT_INSTALLED);
		}
		
		TransportStatus status = TransportStatus.Unknown;

		String command = "sudo -u rtws /usr/bin/jps | grep TransportManager";
	...
	public PipelineStatus getPipelineStatus() throws ServerException {
		...
		if (numProcesses == 0) {
			numProcesses = countProcesses();
		}
		
		PipelineStatus pipelineStatus;
		if (SoftwareUtil.isInactive(SoftwareUtil.TRANSPORT_LOGS_DIR, SoftwareUtil.TRANSPORT_INACTIVE_LOGS_FILENAME, numProcesses)) {
			pipelineStatus = PipelineStatus.INACTIVE;
			logger.info("Transport is inactive.");
		}
		else {
	...
	public int countProcesses() throws ServerException {
			
		if (! SoftwareUtil.isTransportInstalled()) {
			throw new ServerException(TRANSPORT_NOT_INSTALLED);
		}
		
		String command = "grep -o 'transport-services xsi:type' " + 
								SoftwareUtil.TRANSPORT_CONF_DIR + "/" + SoftwareUtil.TRANSPORT_CONFIG_FILENAME + " | wc -l";
		Process process = null;
		String response = null;
		InputStream is = null;
		
		try {
			logger.debug("Executing check transport process num command " + command + "'");
			...
			if (exitValue == 0) {
				response = IOUtils.toString(is,"UTF-8").trim();
			}
