...
import com.deleidos.rtws.core.beans.XMLInstantiationProxy;
...
import com.deleidos.rtws.systemcfg.util.DatasinkProcessingServicesBuilder;
...
public class DataSinkIngestDefinitionComposer implements DefinitionComposer{
	
	private static Logger logger = LogManager.getLogger(DataSinkIngestDefinitionComposer.class);

	private static String PIPELINE_FILENAME = "pipeline";
	private static String SERVICES_FILENAME = "services";
	...
	private JSONArray datasinksJson;
	...
	public void initialize() throws InitializationException {
		...
		templateFile = String.format("%s/%s", baseDir, "rtws-common.properties");
		RtwsConfig.getInstance().getConfiguration().setProperty("sysbuilder.service.default.properties", templateFile);
		
		//set datasink pipeline default template file
		templateFile = String.format("%s/%s", baseDir, "pipeline-template.xml");
		RtwsConfig.getInstance().getConfiguration().setProperty("sysbuilder.datasink.pipeline.template", templateFile);
		
		//set template dir base
		templateFile = String.format("%s/%s", baseDir, "");
		RtwsConfig.getInstance().getConfiguration().setProperty("sysbuilder.template.dir", templateFile);
		
		//setup generic REST client
	...
	/**
	 * Loads default files and properties that the composer will use.
	 */
	@Override
	...
	/**
	 * Compose generates the configuration files for the system configuration for files dealing with data sinks.
	 */
	@Override
	...
	/**
	 * Get Datasinks information from Ingest API.
	 * 
	...
	/**
	 * Get datasink JSONObject from JSONArray datasinks list.
	 * 
	 * @param datasinkName String datasinkName
	 * @return {@link JSONObject}
	 */
	private JSONObject getDatasink(String datasinkName){
		JSONObject json = null;
		
		Iterator<?> nodeit = datasinksJson.iterator();
		while (nodeit.hasNext()) {
			JSONObject tmp = (JSONObject) nodeit.next();
			
			if(((String) tmp.get("fqn")).contains(datasinkName)){
				json = tmp;
				break;
			}
		}
	...
	/**
	 * Recursively replace parameters in an xml template string.
	 * 
	 * @param dataSink {@link DataSink}
	 * @param tmpParser {@link XMLParser}
	 * @param element {@link Node}
	 * @param nodeName String value
	 * @return {@link XMLParser}
	 */
	private XMLParser replaceTemplateParameters(DataSink dataSink, XMLParser tmpParser, Node element, String nodeName){
		//if user parameters exist for data sinks
		if(dataSink.getParams() != null){
			for(Pair pair : dataSink.getParams()){
				if(element != null){
					//for each node recurse
					for(int i = 0; i < element.getChildNodes().getLength(); i++){
						tmpParser = replaceTemplateParameters(dataSink, tmpParser, element.getChildNodes().item(i), element.getChildNodes().item(i).getNodeName());
					}
					if(pair.getName().equals(nodeName)){
						if( p.matcher(element.getTextContent()).matches()){
							properties.put(pair.getName(), pair.getValue());  //add to properties
						}
						element.setTextContent(pair.getValue());
					}
				}//end if
				//for each node check attributes, check if parameter is an attribute
				NodeList nodeList = element.getChildNodes();
				for(int i = 0; i < nodeList.getLength(); i++){
					Node node = nodeList.item(i);
					NamedNodeMap map = node.getAttributes();
					if(map != null){
						Node attrNode = map.getNamedItem(pair.getName());
						if(attrNode != null){
							if(p.matcher(attrNode.getTextContent()).matches()){
								properties.put(pair.getName(), pair.getValue());  //add to properties
							}
							attrNode.setTextContent(pair.getValue());  //set value
						}//end if
					}//end if
				}//end for
			}//end for
	...
	private String buildDatasinkXmlTemplate(String datasinkFqn, JSONObject datasinkJson) {
		StringBuffer xml = new StringBuffer();
		xml.append(startDatasinkXmlTemplate(datasinkFqn));
		xml.append(SystemBuilderUtil.createSystemParametersXml(datasinkJson));
		xml.append(SystemBuilderUtil.createInputParametersXml(datasinkJson));
		xml.append(endDatasinkXmlTemplate());
		return xml.toString();
	}

	private String startDatasinkXmlTemplate(String datasinkFqn) {
		StringBuffer xml = new StringBuffer("\t<pipeline-definition xmlns:xsi=\"http://www.w3.org/2001/XMLSchema-instance\">\n<sink\n");
		xml.append("\txsi:type=\"java:");
		xml.append(datasinkFqn);
		xml.append("\">\n");
		return xml.toString();
	}

	private String endDatasinkXmlTemplate() {
		return "</sink>\n</pipeline-definition>";
	}
	
	private void ifTopicExistsSetProperty(DataSink dataSink){
		for(Pair pair : dataSink.getParams()){
			if("topic".equals(pair.getName())){
				if(StringUtils.isNotBlank(pair.getValue())){
					properties.put("messaging.topic.alert.name", pair.getValue());
				}
				break;  //exit loop
			}
		}//end for
