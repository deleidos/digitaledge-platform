 ...
 */
package com.deleidos.rtws.commons.util.env;
...
public final class SystemEnvUtil {
	...
	public static boolean isTmsSystem() {
		return UserDataProperties.isTmsSystem();
	}
	
	public static boolean isSingleTmsSystem() {
		if (isTmsSystem()) {
			if (UserDataProperties.getInstance().getString(UserDataProperties.RTWS_FQDN,"UNKNOWN").startsWith("tmsp."))
				return true;
		}
	...
	public static boolean isInApplianceMode() {
		return RtwsConfig.getInstance().getString("rtws.cloud.provider", "UNKNOWN").equals("APPLIANCE");
	}

	public static boolean isMarketPlaceLaunched() {
		return Boolean.valueOf(UserDataProperties.getInstance().getString("RTWS_MARKETPLACE_LAUNCHED", "false"));
	}
}