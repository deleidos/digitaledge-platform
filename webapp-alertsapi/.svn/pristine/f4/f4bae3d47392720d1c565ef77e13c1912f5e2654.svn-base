 ...
 */
package com.deleidos.rtws.webapp.alertsapi.example;
...
/**
 * This is an example of how to programmatically invoke the alertsapi. This file
 * is also used in the Alerts API documentation. 
 */
public class AlertsApiUserRestClient {
	...
	public AlertsApiUserRestClient() {
		if (ALERTSAPI_BASE_URL == null) {
			// Set the url of the Alerts API so the REST call knows where the
			// API is
			try {
				RtwsConfig conf = RtwsConfig.getInstance();
				ALERTSAPI_BASE_URL = conf
						.getString("webapp.alertsapi.url.path");
	...
	public String[] getEmailList(String username) throws RestClientException {
		// Create the resource to access the REST Service based on the Alerts API url and REST Service path
	...
	public void updateEmailList(String username, String emailList)
		...
		String response = resource.type(
				MediaType.APPLICATION_FORM_URLENCODED_TYPE).put(String.class,
				f);

		// Process if the response was a success
		JSONObject jsonResponse = JSONObject.fromObject(response);
		JSONObject standardHeader = jsonResponse
				.getJSONObject("standardHeader");
