...
if [ ! -e "$RTWS_MANIFEST" ]; then
  if [ -z "$RTWS_RELEASE" ]; then
    ...
    if [ "$RTWS_MOUNT_MODE" = "s3cmd" ]; then
		...
		s3cmd -c /home/rtws/.s3cfg -f get s3://$RTWS_MOUNT_DEVICE/$RTWS_RELEASE/release/saic-rtws-commons-config.tar.gz /mnt/appfs/release/$RTWS_RELEASE/
    ...
    tar --directory=/usr/local/rtws/boot -xvzf /mnt/appfs/release/$RTWS_RELEASE/saic-rtws-commons-config.tar.gz commons-config/src/templates/manifest/$RTWS_MANIFEST
