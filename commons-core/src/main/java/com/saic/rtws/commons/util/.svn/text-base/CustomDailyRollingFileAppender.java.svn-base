...
public class CustomDailyRollingFileAppender extends FileAppender {
	...
	/**
	 *  This method will iterate through the previous log files and delete the oldest files until the
	 *  maxBackupIndex is reached.  This feature can be disabled by specifying a maxBackupIndex of -1 or
	...
	void deleteOldLogFiles() {
		...
		int startIndex = maxBackupIndex + 1;
		
		if(maxBackupIndex >= 0) // If not set, then will keep all back up files.
		{
...
class ModifiedTimeSortableFile extends File implements Serializable, Comparable<File>
{
	...
	public int compareTo(File anotherPathName) {
		return (int) (anotherPathName.lastModified() - lastModified());
	}
}
