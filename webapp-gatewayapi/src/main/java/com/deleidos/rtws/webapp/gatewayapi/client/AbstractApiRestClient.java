...
public abstract class AbstractApiRestClient {
	...
	protected AbstractApiRestClient() {
		...
		if (RTWS_TMS_DNS == null) {
			try {
				// This is for the gateway instance that is running in the tenant system.
				
				RTWS_TMS_DNS = UserDataProperties.getInstance().getString(UserDataProperties.RTWS_TMS_DNS);
			} catch (ConversionException e) {
				logger.fatal("Load env property [RTWS_TMS_DNS] failed. Message: " + e.getMessage());
			}
		}
		
		if (RTWS_TMS_DNS == null) {
			try {
				// This is for the gateway instance that is running in the tms system.
				
				RTWS_TMS_DNS = RtwsConfig.getInstance().getString("webapp.tenantapi.url.host");
			} catch (ConversionException e) {
	...
	protected String buildBaseUrl(String hostname, String path) {
	...
	protected void processStandardResponse(String response) throws RestClientException {
		...
		if (standardHeader.getInt("code") != 200) {
			...
			if (json.has("responseBody")) {
				...
				message = "Rest client invocation failed.";
			}
