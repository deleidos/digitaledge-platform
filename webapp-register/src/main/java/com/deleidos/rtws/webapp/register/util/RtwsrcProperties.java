...
public class RtwsrcProperties {

	private static final Logger logger = Logger.getLogger(RtwsrcProperties.class);
	
	public static final String RTWS_ACCESS_KEY = "RTWS_ACCESS_KEY";
	public static final String RTWS_ACCT_ID = "RTWS_ACCT_ID";
	public static final String RTWS_AVAILABILITY_ZONE = "RTWS_AVAILABILITY_ZONE";
	...
	private static class RtwsrcPropertiesSingletonHolder { 
		static final RtwsrcProperties instance = new RtwsrcProperties();
	}

	public static RtwsrcProperties getInstance() {
		return RtwsrcPropertiesSingletonHolder.instance;
	}
	
	private RtwsrcProperties() {
		
		FileInputStream fis = null;
		try {
			fis = new FileInputStream("/etc/rtwsrc");
			props.load(fis);
		} catch (IOException e) {
			logger.error("Failed to load user data from /etc/rtwsrc", e);
		}finally{
			if(fis!=null)
				try {
					fis.close();
				} catch (IOException e) {}//eh, whatever
		}
	...
	public String getString(String propertyName) {
		...
		if (value == null || value.trim().length() == 0) {
			logger.warn("Property Name: " + propertyName + " not found in /etc/rtwsrc");
		}
