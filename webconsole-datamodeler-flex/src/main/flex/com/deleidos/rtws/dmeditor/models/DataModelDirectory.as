...
package com.deleidos.rtws.dmeditor.models
{
	...
	public class DataModelDirectory
	{
		...
		public function addDataModelEntry(name:String, visibility:String, versionStrings:Array):void {
			...
			dmRoot.@name = name;
			versionStrings.sort(sortVersions);		// Sort the versions by Major/Minor versioning to prevent lexicographical sorting
			for each (var versionString:String in versionStrings) {
				var versionLeaf:XML = <version/>;
				versionLeaf.@name = name;
				versionLeaf.@version = versionString;
				dmRoot.appendChild(versionLeaf);
				_dataModelNames.push(new DataModelName(name, visibility, versionString));
			}
			_dataModelList.addItem(dmRoot);
		}
		
		private function sortVersions(a:String, b:String):Number {
			var aVersionParts:Array = a.replace("v","").split(".");
			var aMajorVersion:Number = new Number(aVersionParts[0]);
			var aMinorVersion:Number = new Number(aVersionParts[1]);
			
			var bVersionParts:Array = b.replace("v","").split(".");
			var bMajorVersion:Number = new Number(bVersionParts[0]);
			var bMinorVersion:Number = new Number(bVersionParts[1]);
			
			var compareResult:Number = 0;
			if(aMajorVersion < bMajorVersion) {
				compareResult = -1;
			}
			else if(bMajorVersion < aMajorVersion) {
				compareResult = 1;
			}
			else {
				// aMajorVersion == bMajorVersion
				if(aMinorVersion < bMinorVersion) {
					compareResult = -1;
				}
				else if(bMinorVersion < aMinorVersion) {
					compareResult = 1;
				}
			}
