...
/*		if (scriptFiles != null) {
			for (ScriptFile scriptFile : scriptFiles) {
				try {
					...
					String error = "Failed to load the script: " + scriptName + " Message:" + ex.getMessage();
					errors.append(error);
					log.error(error);
				}
			}
			if (errors.length() > 0) {
				throw new ScriptException(errors.toString());
			}
		} */
		...
		if (scriptFiles != null) {
			...
			for (int i = 0; i < scriptFiles.length; i++) {
				try {
					scriptName = scriptFiles[i].getScriptName();
					FileReader fileReader = new FileReader(
							scriptFiles[i].getScriptDirectory() + File.separator
									+ scriptName);
					scriptEngine.eval(fileReader);
				} catch (ScriptException ex) {
					// remove script from array, need to subtract from index so that a value isn't skipped
					scriptFiles = (ScriptFile[]) ArrayUtils.remove(scriptFiles, i--);
					log.warn("Failed to load the script: " + scriptName + " Message:" + ex.getMessage());
				} catch (Exception ex) {
					// for other types of exceptions we still rethrow
					String error = "Failed to load the script: " + scriptName + " Message:" + ex.getMessage();
