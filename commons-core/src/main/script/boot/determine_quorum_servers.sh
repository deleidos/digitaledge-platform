...
if [ -z "$(cat $RTWS_MANIFEST | grep setup_zookeeper.sh)" ]; then
	echo "Updating default zoo.cfg"
	perl -i -pe "s/^server\.0=localhost:2888:3888//g" /etc/zookeeper/zoo.cfg
	
	x=1
	for srv in $(echo $RTWS_ZOOKEEPER_QUORUM_SERVERS | tr -s ',' ' ')
	do
		echo "server.${x}=${srv}:2888:3888" >> /etc/zookeeper/zoo.cfg
		(( x += 1 ))
	done
fi