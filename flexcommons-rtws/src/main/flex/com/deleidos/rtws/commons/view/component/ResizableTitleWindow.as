...
package com.deleidos.rtws.commons.view.component
{
    ...
    import com.deleidos.rtws.commons.view.skin.ResizableTitleWindowSkin;
     ...
     *  a resize handle.
     */
		[Style(name="horizontalScrollPolicy", inherit="no", type="String", enumeration="on, off, auto")]
		[Style(name="verticalScrollPolicy", inherit="no", type="String", enumeration="on, off, auto")]
    public class ResizableTitleWindow extends TitleWindow
    {
        ...
        private var clickOffset:Point;
				...
				private var _verticalScrollPolicy:String = ScrollPolicy.OFF;
        ...
        //--------------------------------------------------------------------------
				...
				[Bindable] public function get horizontalScrollPolicy():String
				{
				...
				public function set horizontalScrollPolicy(value:String):void
				{
					...
					{
						_horizontalScrollPolicy = toValidScrollPolicyValue(value);
					}
				}
				
				[Bindable] public function get verticalScrollPolicy():String
				{
				...
				public function set verticalScrollPolicy(value:String):void
				{
					...
					{
						_verticalScrollPolicy = toValidScrollPolicyValue(value);
					}
				}
        ...
        protected function resizeHandle_mouseMoveHandler(event:MouseEvent):void
        {
            ...
            if (!clickOffset)
            {
                return;
						...
						var isSizeDirty:Boolean = false;
						
						var desiredWidth:Number = prevWidth + (event.stageX - clickOffset.x);
						var hasMinWidth:Boolean = (this.explicitMinWidth && !isNaN(this.explicitMinWidth));
						if(!hasMinWidth || (hasMinWidth && desiredWidth >= this.explicitMinWidth))
						{
							width = desiredWidth;
							isSizeDirty = true;
						}
						
						var desiredHeight:Number = prevHeight + (event.stageY - clickOffset.y);
						var hasMinHeight:Boolean = (this.explicitMinHeight && !isNaN(this.explicitMinHeight));
						if(!hasMinHeight || (hasMinHeight && desiredHeight >= this.explicitMinHeight))
						{
        ...
        protected function resizeHandle_mouseUpHandler(event:Event):void
        {
            ...
            sbRoot.removeEventListener(
				...
				private function toValidScrollPolicyValue(value:String, lienent:Boolean=true):String
				{
					var result:String = null;
					
					if(StringUtils.isNotBlank(value)) {
						var toCheck:String = value;
						if(lienent) {
							toCheck = StringUtils.trim(value);
						}
						
						if(ObjectUtil.stringCompare(toCheck, ScrollPolicy.AUTO, lienent) == 0) {
							result = ScrollPolicy.AUTO;
						}
						else if(ObjectUtil.stringCompare(toCheck, ScrollPolicy.OFF, lienent) == 0) {
							result = ScrollPolicy.OFF;
						}
						else if(ObjectUtil.stringCompare(toCheck, ScrollPolicy.ON, lienent) == 0) {
							result = ScrollPolicy.ON;
						}
					}
					
					if(result == null) {
						throw new ArgumentError("Invalid scroll policy"); 
					}
