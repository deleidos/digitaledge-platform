...
public class IngestDefinitionComposer implements DefinitionComposer{

	private static final Logger logger = LogManager.getLogger(IngestDefinitionComposer.class);
	...
	/**
	 * Initializes the ingest composer resources.
	 */
	@Override
	public void initialize() throws InitializationException {
		try{
			...
			templateFile = String.format("%s/%s", baseDir, "services.parse-enrich-template.xml");
			RtwsConfig.getInstance().getConfiguration().setProperty("sysbuilder.service.parse.enrich.template", templateFile);
		}
		catch(Exception e){
	...
	/**
	 * Loads default resources for the composer.
	 */
	@Override
	public void loadDefaults(String version, SystemConfig config, SystemContext systemContext, RtwsProperties properties)
		...
		try{
			...
			SystemBuilderUtil.mkdir(this.outDir);  //create director
		}
		catch(Exception e){
			logger.error(e.toString(), e);
			e.printStackTrace();
	...
	/**
	 * Creates the Ingest related configuration files.
	 */
	@Override
	...
	/**
	 * Generates the services.parse-enrich.xml file configuration.
	 * 
	 * @throws IOException
	 */
	public void generateParseEnrich() throws IOException{
		InputStream is = null;
		try{
			//get file template path
			String parseEnrichTemplate = RtwsConfig.getInstance().getString("sysbuilder.service.parse.enrich.template");
			
			//read in template file
			BufferedReader br = new BufferedReader(new InputStreamReader(getClass().getResourceAsStream(parseEnrichTemplate)));
			StringBuilder sb = new StringBuilder("");  //enrich parse template
			...
			//recursively replace properties in parse enrich template
			...
			this.parser.writeXmlFile(outDir + "services.parse-enrich.xml");
		}
		//cleanup
		finally{
			is.close();
