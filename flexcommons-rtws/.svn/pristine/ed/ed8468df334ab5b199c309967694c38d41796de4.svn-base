...
package com.deleidos.rtws.commons.model.datamodel
{
	public class DataModelName
	{
		 ...
		 */
		public function DataModelName(name:String, visibility:String, version:String=null) {
			if (version == null) {
				var uscore:int = name.lastIndexOf("_");
				_name = name.substr(0,uscore);
				name = name.replace(/\.zip$/, "");
				_version = name.substr(uscore+1);
			}
			else {
				_name = name;
				_version = version;				
		...
		public function get zipFilename():String {
			return this.getZipFilename();
		}
