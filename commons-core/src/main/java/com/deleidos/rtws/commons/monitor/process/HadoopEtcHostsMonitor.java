...
public class HadoopEtcHostsMonitor extends ProcessMonitor {
	...
	protected void monitor() {
		try {
			...
			if (StringUtils.isNotBlank(provider) == true) {
				...
				if (hostnamesToApply.size() == 0) {
					logger.info("Assuming that this system does not contain a managed hadoop cluster, so bypassing /etc/hosts modification.");
