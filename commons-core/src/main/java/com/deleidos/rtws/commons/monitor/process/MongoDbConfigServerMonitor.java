...
public class MongoDbConfigServerMonitor extends ProcessMonitor {
	...
	public MongoDbConfigServerMonitor(String name) {
		super(name);
	...
	protected void monitor() {
		...
		case Running:
			if (!process.testConnection(this.info.hostname, this.info.port)) {
				addError("Failed MongoDB Config Server connection test.");
				break;
			}
			setStatus(MonitorStatus.OK);
			break;
		case Stopped:
			addError("MongoDB Config Server is not running.");
			break;
		default:
			addError("MongoDB Config Server status is currently unknown.");
			break;
		}
	}
