...
dd if=/dev/zero of=/mnt/swapfile bs=1024 count=6048500
...
# TODO remove this assumption that this mount point exists as it does on the nodes currently used in AWS for SGS system
mkdir -p /mnt2
mount /dev/xvdc /mnt2

for dir in /mnt/mapred/local /mnt2/mapred/local
do
	if [ ! -d ${dir} ]; then
...
# Ensure the cluster balancer runs daily
cat >>/etc/cron.daily/run_balancer<<EOF
#!/bin/bash
su hdfs -l -s /bin/bash -c "hdfs balancer > /tmp/.balancer.out"
EOF
chmod +x /etc/cron.daily/run_balancer
fromdos /etc/cron.daily/run_balancer
