...
public class ConfigEncryptor implements StringEncryptor {
	...
	public static synchronized ConfigEncryptor instance() {
		if (_instance == null) {
			_instance = new ConfigEncryptor(EncryptorUtil.getEncryptionPassword());
		}
		return _instance;
	}
	
	public ConfigEncryptor(String var) {
//		Run jasypt's listAlgorithms.sh to see list of available algorithms, java built in algorithms below
//		DIGEST ALGORITHMS:   [MD2, MD5, SHA, SHA-256, SHA-384, SHA-512]
//		PBE ALGORITHMS:      [PBEWITHMD5ANDDES, PBEWITHMD5ANDTRIPLEDES, PBEWITHSHA1ANDDESEDE, PBEWITHSHA1ANDRC2_40]
//		The built in algorithms are considered weak at this point, so we are using Bouncy Castle for AES  https://www.bouncycastle.org/java.html
//		If not running on OpenJDK, you will need to install the JCE unlimited strength policies	
		this.encryptor = new StandardPBEStringEncryptor();
		this.encryptor.setProvider(new BouncyCastleProvider());
		this.encryptor.setAlgorithm("PBEWITHSHA256AND128BITAES-CBC-BC");
		this.encryptor.setPassword(var);
	...
	public String encryptWithWrapper(String plain) {
		String black = plain;
		try {
			black = PropertyValueEncryptionUtils.encrypt(plain, this);
	...
	public String decryptWithWrapper(String black) {
		...
		try {
			if (PropertyValueEncryptionUtils.isEncryptedValue(black)) {
				plain = PropertyValueEncryptionUtils.decrypt(black, this);
			}
		} catch (EncryptionOperationNotPossibleException e) {
	...
	public static String wrap(String black) {
		return String.format("%s%s%s", "ENC(", black, ")");		
	}

	public static String unwrap(String black) {
		if (black.startsWith("ENC(") && black.endsWith("")) {
			return black.substring(4, black.length()-1);
		} else {
			return black;
		}
	}
	...
	public static void main(String[] args) {
		...
		for (int i=0; i < args.length; i++) {
			...
			if (decryptOnly) {
				ConfigEncryptor confEncrypt = new ConfigEncryptor(args[i]);
				i = i + 1;
				String black = args[i];
				String plain = confEncrypt.decrypt(black);
				if (verbose) {
				...
				i = i + 1;
				String plain = args[i];
				String black = confEncrypt.encrypt(plain);
				if (verbose) {
					System.out.println("Encrypted value of [" + plain + "] is [" + black + "]");
				} else {
					System.out.println(black);
				}
				String decrypt = confEncrypt.decrypt(black);
				if (verbose) {
