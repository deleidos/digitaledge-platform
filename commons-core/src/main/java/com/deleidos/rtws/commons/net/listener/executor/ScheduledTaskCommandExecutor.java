...
import com.deleidos.rtws.commons.net.listener.model.ScheduleClient;
import com.deleidos.rtws.commons.net.listener.model.ScheduleJob;
import com.deleidos.rtws.commons.net.listener.process.JettyProcess;
...
public class ScheduledTaskCommandExecutor extends CommandExecutor {
	...
	public ExecuteResult execute() throws ServerException {
		...
		JettyRestarter r = new JettyRestarter();
		r.start();
		
		return buildTerminateResult(ResponseBuilder.buildSuccessResponse());
	}
	
	private class JettyRestarter extends Thread{
		
		@Override
		public void run(){
			try {
				JettyProcess.newInstance(false).restart();
			} catch (ServerException e) {
				logger.debug("Error restarting jetty/sync repo",e);
			}
		}
	}
}