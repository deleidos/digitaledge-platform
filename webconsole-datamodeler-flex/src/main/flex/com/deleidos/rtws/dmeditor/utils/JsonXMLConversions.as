...
package com.deleidos.rtws.dmeditor.utils
{
	...
	import com.deleidos.rtws.dmeditor.json.JsonItem;
	import com.deleidos.rtws.dmeditor.json.JsonLiteral;
	import com.deleidos.rtws.dmeditor.json.JsonObject;
	...
	public class JsonXMLConversions
	{
		...
		private static function isArrayWithXlateMarker(field:XML):Boolean {
			var result:Boolean = false;
			var childList:XMLListCollection = new XMLListCollection(field.children());
		...
		private static function walkXML(xmlList:XML, useXlate:Boolean):String {
			...
			for each (var field:XML in xmlList) {
				...
				if (fieldtype == "OBJECT") {
					...
					if (useXlate && !StringUtils.isEmpty(field.@xlate) && !isArrayWithXlateMarker(field)) {
		...
		/**
		 * Return only what we want for the parameters. Custom editor params
		 * are not returned.
		 */
		private static function makeParserParamsArray(paramsArray:Array):Array {
			var result:Array = new Array();
			for each (var param:ParserParameter in paramsArray) {
				if (!param.hasCustomEditor) {
					result.push(param.transferObject);
				}
			}
			return result;
		}
		...
		public static function buildDataSourcesJSON(dataSources:Array):String {
			...
			for each (var dataSource:Object in dataSources) {
				...
				obj.parserParams = makeParserParamsArray(parserParams);
		...
		public static function buildCustomParserParams(dataSources:Array):void {
		...
		public static function buildEnrichmentJSON(enrichments:Array):String {
			...
			for each (var enrichment:Object in enrichments) {
				...
				json += JSON.encode(EnrichmentModel.makeTransferObject(enrichment));
				first = false;
			}
