...
import com.deleidos.rtws.commons.net.listener.process.MongoDbProcess;
...
public class MongoDbMonitor extends ProcessMonitor {
	...
	private void loadMongoDbPipelineDefinition() throws Exception {		
		...
		if (new File(MONGODB_PIPELINE_XML_FILEPATH).exists()) {			
			...
			// Since ingest doesn't run, mark as completed so process group monitor completes startup
			new File(ContentDownloader.COMPLETION_FILE_PATH).createNewFile();
		} else {
			logger.warn("Assuming standlone mongodb process group");
			this.info.hostname = "mongodb." + UserDataProperties.getInstance().getString(UserDataProperties.RTWS_DOMAIN);
		}		
	}
