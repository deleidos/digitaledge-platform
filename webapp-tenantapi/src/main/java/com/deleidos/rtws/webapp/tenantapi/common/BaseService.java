...
public class BaseService {
	...
	protected StandardResponse<?> buildStandardResponse(String jsonString) {
		JSONObject jsonResponse = JSONObject.fromObject(jsonString);
		JSONObject jsonStandardHeader = jsonResponse.getJSONObject("standardHeader");
		
		int code = jsonStandardHeader.getInt("code");
		if (code == 200) {
			PropertiesResponse response = new PropertiesResponse();
			response.setStandardHeaderCode(200);
			
			JSONObject jsonResponseBody = jsonResponse.getJSONObject("responseBody");
			
			@SuppressWarnings("rawtypes")
			Iterator it = jsonResponseBody.keys();
			while (it.hasNext()) {
				String key = (String) it.next();
				String value = jsonResponseBody.getString(key);
	...
	protected PropertiesResponse buildPropertiesResponse(
			int code, 
			String key, 
			String value) {
		return buildPropertiesResponse(code, Collections.singletonMap(key, value));
	}
	...
	protected WebApplicationException buildWebApplicationException(String errorMsg) {
