...
import com.deleidos.rtws.commons.util.expression.Matcher;
import com.deleidos.rtws.commons.util.expression.Parser;

/**
 * Matcher that evaluates whether a given value is less than or equal to the configured value;
 */
public class NotGreater implements Matcher {

	private static Parser parser = Parser.compile("\\s*<=\\s*(\\l)\\s*");
	
	private Object criteria;
	
	public NotGreater(Object criteria) {
	...
	public static NotGreater valueOf(String expression) {
		return new NotGreater(parser.parse(expression).getTermAsLiteral(0));
	}
