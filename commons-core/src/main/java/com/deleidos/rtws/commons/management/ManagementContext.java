 ...
 */
package com.deleidos.rtws.commons.management;
...
import com.deleidos.rtws.commons.management.rmi.ssl.SslContextAwareRMIClientSocketFactory;
import com.deleidos.rtws.commons.management.rmi.ssl.SslContextAwareRMIServerSocketFactory;
...
public class ManagementContext implements Initializable {
	...
	static {
		...
		String rmiPort = System.getProperty("com.deleidos.rtws.management.rmiPort", "1099");
		String jmxPort = System.getProperty("com.deleidos.rtws.management.jmxPort", "61516");
		...
		try {
			// Create an environment that uses custom SslContext configured connections
			String keyStore = RtwsConfig.getInstance().getString("listener.keystore");
			String keyStorePassword = RtwsConfig.getInstance().getString("listener.keystore.password");
			String trustStore = RtwsConfig.getInstance().getString("listener.truststore");
			String trustStorePassword = RtwsConfig.getInstance().getString("listener.truststore.password");
			
			SslContextAwareRMIClientSocketFactory sslContextAwareCsf = new SslContextAwareRMIClientSocketFactory(keyStore, keyStorePassword, trustStore, trustStorePassword);
			SslContextAwareRMIServerSocketFactory sslContextAwareSsf = new SslContextAwareRMIServerSocketFactory(keyStore, keyStorePassword, trustStore, trustStorePassword);

			HashMap<String, Object> environment = new HashMap<String, Object>();
			environment.put(RMIConnectorServer.RMI_CLIENT_SOCKET_FACTORY_ATTRIBUTE, sslContextAwareCsf);
			environment.put(RMIConnectorServer.RMI_SERVER_SOCKET_FACTORY_ATTRIBUTE, sslContextAwareSsf);
			...
			JMXConnectorServerFactory.newJMXConnectorServer(url, environment, registry).start();
