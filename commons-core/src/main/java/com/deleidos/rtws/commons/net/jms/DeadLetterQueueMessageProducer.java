...
public class DeadLetterQueueMessageProducer extends AbstractMessageProducer {
	...
	public DeadLetterQueueMessageProducer(ConnectionFactory factory) {
		super(factory);
		this.queueName = RtwsConfig.getInstance().getString("messaging.queue.dlq.name", "com.deleidos.rtws.DLQ?consumer.prefetchSize=0");
	}
	...
	public void start() throws JMSException {
    	...
    	while (consumer == null && (System.currentTimeMillis()-startTime)< 3*60*1000) {
    		try {
	    		super.open();
	    		connection.start();
    			...
    			super.close();
    			log.error("Unable to connect to DeadLetterQueue");
