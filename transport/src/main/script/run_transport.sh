...
# RTWS_TRANSPORT_CONFIG - The name of the configuration file with which to start the transport process.
#
# Optional parameters.
# RTWS_TRANSPORT_OPTIONS - May be used to specify JVM command line parameters.

tmp=`cat /etc/rtwsrc | grep RTWS_SECRET_KEY | cut -f2 -d"="`
secretKey="secretKey=$tmp"
tmp=`cat /etc/rtwsrc | grep RTWS_ACCESS_KEY | cut -f2 -d"="`
accessKey="accessKey=$tmp"

echo "$secretKey"  > /usr/local/rtws/transport/conf/aws.properties
echo "$accessKey"  >> /usr/local/rtws/transport/conf/aws.properties

chown rtws:rtws /usr/local/rtws/transport/conf/aws.properties
chmod 644 /usr/local/rtws/transport/conf/aws.properties
...
if [ ! -d /usr/local/rtws/transport/logs ]; then
	mkdir -p /usr/local/rtws/transport/logs
fi

if [ ! -d /tmp/transport/lib ]; then
    mkdir -p /tmp/transport/lib 
fi

if [ ! -d /tmp/transport/scripts ]; then
    mkdir -p /tmp/transport/scripts 
fi
...
# Download services from the repository
java $RTWS_TRANSPORT_OPTIONS -cp "../lib/*" com.deleidos.rtws.transport.tools.TransportContentDownloader ../conf/transport-content-downloader.xml 30
...
if [ -f /usr/local/rtws/transport/bin/env/pre_start ]; then
	...
	/usr/local/rtws/rtws/transport/bin/env/pre_start
fi

# Startup the transport
java $RTWS_TRANSPORT_OPTIONS -cp "../lib/*" com.deleidos.rtws.transport.TransportManager ../conf/transports.default.xml
